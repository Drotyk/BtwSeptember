...
export function validateNotificationInput(input: CreateNotificationInput): string | null {
  ...
  if (!message || message.length > 4000) return 'Текст повідомлення обов\'язковий (до 4000 символів)';
  ...
  if ((input.targetType === 'user' || input.targetType === 'custom') && 
  ...
  if (input.expiresAt !== undefined) {
    ...
    if (input.expiresAt <= input.scheduledAt) return 'Дата закінчення має бути після дати відправки';
  }
